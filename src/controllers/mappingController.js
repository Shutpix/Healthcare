...
exports.assignDoctor = async (req, res) => {
  try {
    ...
    if (existing) return res.status(409).json({ error: "Doctor already assigned to this patient" });

    const mapping = await Mapping.create({ patientId, doctorId });

    const fullMapping = await Mapping.findById(mapping._id)
      .populate({ path: 'doctorId', select: 'name specialization -_id' })
      .populate({ path: 'patientId', select: 'name condition -_id' });

    res.status(201).json({
      message: "✅ Doctor assigned to patient successfully",
      doctor: fullMapping.doctorId,
      patient: fullMapping.patientId
    });
